...
function posterior_distr(povms, rhos, precision = 4)
    res = []
    N = length(povms)[1]
    for i in 1:N
        append!(res, round(real(LinearAlgebra.tr(rhos[i] * povms[i])), digits = precision))
    end
    return res
end

function distr_non_uniformity(distr)
    @assert(round(sum(distr)) == 1)
    N = length(distr)
    unif = [1/N for i in 1:N]
    v = LinearAlgebra.vec(distr) - LinearAlgebra.vec(unif)
...
vecs_to_kets([[0,0,1], [1,0,0], [-1/2,sqrt(3)/2,0], [-1/2,-sqrt(3)/2,0], [0,0,-1]])

# distr_non_uniformity([0, 0, 1/2, 1/2])